...
import { X, Github, Download, AlertCircle, FileJson, Check, ExternalLink, RefreshCw, Save, FolderGit2, Search, GitBranch, Loader2 } from 'lucide-react';
...
export const GitHubSyncModal: React.FC<GitHubSyncModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (savedMode === 'repo' || savedMode === 'gist') setMode(savedMode as 'repo' | 'gist');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100 dark:border-slate-800 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Existing Gist ID to sync with. Leave empty to create a new one.
                </p>
              </div>
              ...
              {onToggleAutoSync && (
                <div className="flex items-center justify-between p-3 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${autoSyncEnabled ? 'bg-indigo-100 dark:bg-indigo-800 text-indigo-600 dark:text-indigo-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                      <RefreshCw size={18} className={autoSyncEnabled ? "animate-spin-slow" : ""} />
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200">Auto-Sync via Gist</h4>
                      <p className="text-xs text-slate-500 dark:text-slate-400">Automatically save changes to this Gist</p>
                    </div>
                  </div>
                  <button
                    onClick={() => onToggleAutoSync(!autoSyncEnabled)}
                    className={`relative w-11 h-6 rounded-full transition-colors ${autoSyncEnabled ? 'bg-indigo-500' : 'bg-slate-300 dark:bg-slate-600'}`}
                  >
                    <span
                      className={`absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform ${autoSyncEnabled ? 'translate-x-5' : 'translate-x-0'}`}
                    />
                  </button>
                </div>
              )}
          ...
          {/* Feedback Area */}
          {status !== 'idle' && (
            <div className={`p-4 rounded-xl flex items-center gap-3 text-sm font-medium ${status === 'error' ? 'bg-rose-50 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400' :
